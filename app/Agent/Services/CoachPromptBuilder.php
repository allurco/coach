...
/**
 * Builds the system prompt the agent receives. Extracted from
 * CoachAgent in Phase 9 — see ADR 0003 (Agent layer) for why agent-
 * owned services live in app/Agent/Services/, and the Phase 7
 * CoachInteraction precedent for the User-passed-explicitly pattern.
 *
 * The builder is responsible for assembling EVERY section of the
 * system prompt:
 *   - Today / weekend signal
 *   - Voice and tone (locale-aware)
 *   - Locale-specific fiscal/cultural knowledge (markdown loader)
 *   - Life context (pack signals — see ADR 0002)
 *   - Goal context (active goal + specialization + latest "why")
 *   - User context (profile facts from coach_memories)
 *   - Plan stats (action counts)
 *   - Long-term memory (recent CoachMemory entries)
 *   - Onboarding-specific prompt (when no actions exist yet)
 *
 * Callers pass User explicitly — the builder never reads auth().
 * That centralises auth at the entry points (CoachAgent::instructions,
 * tests, future callers) and makes the builder unit-testable without
 * a logged-in session.
 */
class CoachPromptBuilder
{
    /**
     * In-process cache for loaded locale knowledge files. Markdown content
     * is immutable per-deploy — reading from disk every time is wasteful.
     * Keyed by resolved locale (already sanitized in resolveLocale).
     *
     * @var array<string, string>
     */
    protected static array $localeKnowledgeCache = [];

    public function build(User $user, ?int $activeGoalId = null, ?string $conversationId = null): string
    {
        $today = $this->todayContext();
        $tonePersona = $this->tonePersona($user);
        $localeKnowledge = $this->renderLocaleKnowledgeSection($user);
        $userContext = $this->userContext($user);
        $goalContext = $this->goalContext($user, $activeGoalId, $conversationId);
        $isOnboarding = Action::count() === 0;

        if ($isOnboarding) {
            return $this->buildOnboarding($userContext, $today, $goalContext, $tonePersona, $localeKnowledge);
        }

        $stats = $this->planStats();
        $recentMemories = $this->recentMemoriesSummary($user);
        $lifeContext = $this->lifeContext($user, $activeGoalId);

        return $this->buildFull(
            today: $today,
            tonePersona: $tonePersona,
    ...
    protected function buildOnboarding(
        string $userContext,
        string $today,
        string $goalContext = '',
        string $tonePersona = '',
     ...
     *
     * When no pack contributes anything (no enabled packs, or all packs
     * return null), the whole block is omitted.
     *
     * The tool_hint string at the bottom is still finance-flavoured copy
     * living in the coach lang namespace — a known leak documented since
     * Phase 4. Cleaning it up requires a pack contribution mechanism for
     * tool hints, deferred until a second pack pressures the same pattern.
     */
    protected function lifeContext(User $user, ?int $activeGoalId): string
    {
        $activeGoal = $activeGoalId ? Goal::find($activeGoalId) : null;
     ...
     *   1. The goal explicitly set via $activeGoalId
     *   2. The goal_id of the active conversation (if any)
     *   3. The user's most recently-touched non-archived goal
     ...
     */
    protected function goalContext(User $user, ?int $activeGoalId, ?string $conversationId): string
    {
        $goal = $this->resolveActiveGoal($activeGoalId, $conversationId);
    ...
    protected function resolveActiveGoal(?int $activeGoalId, ?string $conversationId): ?Goal
    {
        if ($activeGoalId !== null) {
            return Goal::find($activeGoalId);
        }
    ...
    protected function recentMemoriesSummary(User $user): string
    {
     ...
     */
    protected function tonePersona(User $user): string
    {
        return match ($this->resolveLocale($user)) {
     ...
     */
    protected function localeKnowledge(User $user): string
    {
     ...
     */
    protected function renderLocaleKnowledgeSection(User $user): string
    {
        $knowledge = $this->localeKnowledge($user);
